...
final class DisplayWaiter {
	...
	public final class Timeout {
		...
		/**
		 * Returns <code>true</code> if the timeout has been reached, <code>false</code>
		 * if not.
		 * 
		 * @return <code>true</code> if the timeout has been reached, <code>false</code>
		 *         if not
		 */
	 ...
	 * 
	 * STOPPED -> RUNNING RUNNING -> STOPPED RUNNING -> IDLE IDLE -> RUNNING IDLE ->
	 * STOPPED
	 */
	 ...
	 *            the display to run the event loop of
	 * @param keepRunning
	 *            <code>true</code> if the thread should be kept running after
	 *            timing out
	 */
	...
	/**
	 * Starts the timeout thread if it is not currently running. Nothing happens if
	...
	/**
	 * Starts the thread if it is not currently running; resets the timeout if it
	 * is.
	 * 
	 * @param delay
	 *            the delay from now in milliseconds
	...
	/**
	 * Stops the thread if it is running. If not, nothing happens. Another thread
	 * may be started by calling {@link #start(long)} or {@link #restart(long)}.
	 */
	public void stop() {
		synchronized (fMutex) {
	...
	/**
	 * Puts the reaper thread on hold but does not stop it. It may be restarted by
	 * calling {@link #start(long)} or {@link #restart(long)}.
	 */
	public void hold() {
		synchronized (fMutex) {
	...
	private void startThread() {
		...
		fCurrentThread = new Thread() {
			/**
			 * Exception thrown when a thread notices that it has been stopped and a new
			...
			/**
			 * Check whether the current thread is this thread, throw an exception
			 * otherwise.
			 * 
			 * @throws ThreadChangedException
			 ...
			 * <code>IDLE</code> (if in keep-running mode) or <code>STOPPED</code>.
			 */
			...
			/**
			 * Waits while the state is <code>IDLE</code>, then returns. The state must not
			 * be <code>RUNNING</code> when calling this method. The state is either
			 * <code>STOPPED</code> or <code>RUNNING</code> when the method returns.
			 * 
			 * @throws InterruptedException
			 *             if the thread was interrupted
			 * @throws ThreadChangedException
	 ...
	 * <code>possibleStates</code>. Returns <code>true</code> if the transition
	 * happened, <code>false</code> otherwise.
	 * 
	 * @param possibleStates
	 *            the states which trigger a transition
	 * @param nextState
	 *            the state to transition to
	 * @return <code>true</code> if the transition happened, <code>false</code>
	 *         otherwise
	 */
	...
	/**
	 * Checks the <code>possibleStates</code> and throws an assertion if it is not
	 * met, then transitions to <code>nextState</code>.
	 * 
	 * @param possibleStates
	 *            the allowed states
	 * @param nextState
